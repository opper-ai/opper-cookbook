...
class LiveScoreboard:
    ...
    async def _run_rich_display(self, refresh_hz: float):
        ...
        def _make_display():
            ...
            for name in sorted(self.player_names, key=lambda n: self.scores.get(n, 0.0), reverse=True):
        ...
        with Live(_make_display(), refresh_per_second=refresh_hz, console=console, screen=False) as live:
            try:
                ...
                try:
                    ...
                    if event.event_type == "match_completed":
                        ...
                        if result == "WIN" and winner and loser:
                            ...
                            winner = event.data.get("winner")
                            logger.info(f"Match: Illegal move by {offender}, {winner} awarded win")
